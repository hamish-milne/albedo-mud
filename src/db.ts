...
import type { DeepPartial, EntityType, EventMask, EventType } from "./types.js";
...
)) ${finalFilter ? "WHERE " : ""}${finalFilter ? finalFilter : ""}`;
}
...
type ReadChildEntity<T extends EntityType = EntityType> = Const<
	BaseEntity<T> & Read & Child
>;

type ReadRootEntity<T extends EntityType = EntityType> = Const<
	BaseEntity<T> & Read & Root
>;

type ReadEntity<T extends EntityType = EntityType> = Const<
...
>;
type ReadEvent<T extends EventType = EventType> = Const<
...
>;
type WriteEvent<T extends EventType = EventType> = Const<
